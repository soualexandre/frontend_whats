

export default function UploadDocument() {
    return (
        <>
            <div className="py-4 pt-10">
                <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Enviar um documento</label>
                <input
                    className="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                    aria-describedby="file_input_help"
                    id="file_input"
                    type="file"
                    accept=".csv, .xls, .xlsx"
                />
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">xls, xlsx, csv</p>
            </div>
        </>
    );

}